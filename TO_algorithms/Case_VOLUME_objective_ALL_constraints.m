...
dof_mirror_y = zeros(1, numel(nodx)*numel(nodz));
count_nodes_4 = 1;
for x = 1:numel(nodx)
    for z = 1:numel(nodz)
        dof_mirror_y(1, count_nodes_4) = ((numel(nody)-1)*numel(nodz)*numel(nodx) + (z-1)*numel(nodx) + x)*3-2;
...
dof_fixed = sort(cat(2, dof_fixed, dof_mirror_y));                             % Activate this in case there is mirroring in the y direction
...
options_nodisplay = optimoptions('fmincon','SpecifyConstraintGradient', true, 'SpecifyObjectiveGradient',true,'HessianApproximation', 'lbfgs',  'Display', 'none', ...
   'OutputFcn', @F_outputfun_case, 'MaxIterations', 1000, 'MaxFunctionEvaluations', 700000, ...
...
[x, fval] = fmincon(f, xi, [],[],[],[],lb,ub,constr, options);
