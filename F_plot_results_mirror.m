function F_plot_results_mirror(name_run_L, nelx_L, nely_L, nelz_L, d_FE_L, xi_L, itt_L, neigh_dis_L, neigh_sum_L) %BETA EN PEN weggehaald
%Plotting Function that can include mirroring 
...
%adding in mirror
nelx_L = nelx_L*2;                         % Activate in case Mirroring in X
nely_L = nely_L*2;                          % Activate in case Mirroring in Y
xPhys_mirror_x = flip(xPhys, 2);           % Activate in case Mirroring in X
xPhys = cat(2, xPhys, xPhys_mirror_x);     % Activate in case Mirroring in X
xPhys_mirror_y = flip(xPhys, 3);            % Activate in case Mirroring in Y
xPhys = cat(3,  xPhys, xPhys_mirror_y);     % Activate in case Mirroring in Y

nele_L = nelx_L*nely_L*nelz_L;              % Total amount elements with mirroring included
